...
function getRandom(exclude=[]){
 const starterWrds = storage.games.chainOne.starter
 const words = starterWrds.filter(w => !exclude.includes(w))
    return words[Math.floor(Math.random()*words.length)]
}

async function wordChainS(sock, from, sender, text, isContinuation = false) {
    ...
    if(!isContinuation){
        ...
        storage.games.active[from].timer = setTimeout(async ()=>{
            const gm = storage.games.active[from] 
            await sock.sendMessage(from,{
                text:`⏱Time's up, Game over better luck next time. 
                \n score: 0 | Level: 1`
            })
            storage.games.chainOne.scores[from] = storage.games.chainOne.scores[from] || []
            storage.games.chainOne.scores[from].push({date: Date.now(),scores:{
               [gm.currentPlayer]:gm.score 
            }})
...
if(game.score % 10 === 0){
    game.level += 1
    game.timeLimit = Math.max(3, game.timeLimit - 3)
    await sock.sendMessage(from,{
